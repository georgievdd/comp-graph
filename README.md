...
Метод `parallelProjection()` проецирует 3D точку на плоскость Z=0:

```java
P'(x, y) = P(x, y, z) → (x, y)
```

Это ортогональная проекция - параллельные линии остаются параллельными, расстояния сохраняются.

**Свойства:**
- Отсутствие искажения пропорций
- Параллельные линии остаются параллельными
- Размер объекта не зависит от расстояния до плоскости проекции

#### 4. Перспективная проекция

Метод `perspectiveProjection()` создает одноточечную перспективу с центром в (0, 0, k):

```java
factor = k / (k - z)
P'(x, y) = (x · factor, y · factor)
```

**Свойства:**
- Объекты ближе к наблюдателю выглядят больше
- Параллельные линии, уходящие вдаль, сходятся в точке схода
- Реалистичное восприятие глубины

#### 5. Удаление невидимых ребер (Back-Face Culling)
