...
| Исходные полигоны | Результат отсечения |
|-------------------|---------------------|
| ![Setup](res/hw_clip_orientation_setup.png) | ![Result](res/hw_clip_orientation_result.png) |

**Тест 4: Полигон полностью внутри другого (clip внутри subject):**

| Исходные полигоны | Результат отсечения |
|-------------------|---------------------|
| ![Setup](res/hw_clip_inside_setup.png) | ![Result](res/hw_clip_inside_result.png) |

**Тест 5: Subject полностью внутри clip:**

| Исходные полигоны | Результат отсечения |
|-------------------|---------------------|
| ![Setup](res/hw_clip_subj_inside_setup.png) | ![Result](res/hw_clip_subj_inside_result.png) |

**Тест 6: Нет пересечений и нет вложенности:**

| Исходные полигоны | Результат (пустое пересечение) |
|-------------------|-------------------------------|
| ![Setup](res/hw_clip_no_intersect_setup.png) | ![Result](res/hw_clip_no_intersect_result.png) |

---

### Задание 2: Кривая Эрмита (адаптивная сегментация)

Число сегментов для кусочно-линейной аппроксимации вычисляется адаптивно на основе:
- Расстояния между контрольными точками
- Длин касательных векторов
- Угла между касательными

| Кривая Эрмита | С подписями и числом сегментов |
|---------------|-------------------------------|
| ![Hermite](res/hw_hermite_curve.png) | ![Labeled](res/hw_hermite_curve_labeled.png) |

**Демонстрация адаптивной сегментации:**

![Adaptive Demo](res/hw_hermite_adaptive_demo.png)
